...
_lr_productions = [
  ...
  ('estatuto -> asignacion SEMICOLON','estatuto',2,'p_estatuto','Parser.py',159),
  ('estatuto -> llamada SEMICOLON','estatuto',2,'p_estatuto','Parser.py',160),
  ('estatuto -> retorno SEMICOLON','estatuto',2,'p_estatuto','Parser.py',161),
  ('estatuto -> lectura SEMICOLON','estatuto',2,'p_estatuto','Parser.py',162),
  ('estatuto -> escritura SEMICOLON','estatuto',2,'p_estatuto','Parser.py',163),
  ('estatuto -> decision','estatuto',1,'p_estatuto','Parser.py',164),
  ('estatuto -> while','estatuto',1,'p_estatuto','Parser.py',165),
  ('estatuto -> for','estatuto',1,'p_estatuto','Parser.py',166),
  ('asignacion -> variable ASSIGN expr','asignacion',3,'p_asignacion','Parser.py',173),
  ('llamada -> ID O_PARENTHESIS llamada_prime C_PARENTHESIS','llamada',4,'p_llamada','Parser.py',180),
  ('llamada_prime -> exp','llamada_prime',1,'p_llamada_prime','Parser.py',186),
  ('llamada_prime -> exp COMMA llamada_prime','llamada_prime',3,'p_llamada_prime','Parser.py',187),
  ('retorno -> RETURN O_PARENTHESIS expr C_PARENTHESIS','retorno',4,'p_retorno','Parser.py',194),
  ('lectura -> READ O_PARENTHESIS lectura_prime C_PARENTHESIS','lectura',4,'p_lectura','Parser.py',201),
  ('lectura_prime -> variable','lectura_prime',1,'p_lectura_prime','Parser.py',207),
  ('lectura_prime -> variable COMMA lectura_prime','lectura_prime',3,'p_lectura_prime','Parser.py',208),
  ('escritura -> WRITE O_PARENTHESIS escritura_prime C_PARENTHESIS','escritura',4,'p_escritura','Parser.py',215),
  ('escritura_prime -> expr','escritura_prime',1,'p_escritura_prime','Parser.py',221),
  ('escritura_prime -> CTE_STRING','escritura_prime',1,'p_escritura_prime','Parser.py',222),
  ('escritura_prime -> expr COMMA escritura_prime','escritura_prime',3,'p_escritura_prime','Parser.py',223),
  ('escritura_prime -> CTE_STRING COMMA escritura_prime','escritura_prime',3,'p_escritura_prime','Parser.py',224),
  ('decision -> IF O_PARENTHESIS expr C_PARENTHESIS bloque else','decision',6,'p_decision','Parser.py',231),
  ('else -> ELSE bloque','else',2,'p_else','Parser.py',237),
  ('else -> epsilon','else',1,'p_else','Parser.py',238),
  ('while -> WHILE O_PARENTHESIS expr C_PARENTHESIS bloque','while',5,'p_while','Parser.py',245),
  ('for -> FOR variable ASSIGN exp TO exp bloque','for',7,'p_for','Parser.py',252),
  ('expr -> or','expr',1,'p_expr','Parser.py',259),
  ('or -> and','or',1,'p_or','Parser.py',266),
  ('or -> and OR or','or',3,'p_or','Parser.py',267),
  ('and -> equal','and',1,'p_and','Parser.py',274),
  ('and -> equal AND and','and',3,'p_and','Parser.py',275),
  ('equal -> compare','equal',1,'p_equal','Parser.py',282),
  ('equal -> compare EQUAL compare','equal',3,'p_equal','Parser.py',283),
  ('equal -> compare NOT_EQUAL compare','equal',3,'p_equal','Parser.py',284),
  ('compare -> exp','compare',1,'p_compare','Parser.py',291),
  ('compare -> exp GREATER exp','compare',3,'p_compare','Parser.py',292),
  ('compare -> exp LESSER exp','compare',3,'p_compare','Parser.py',293),
  ('compare -> exp GREATER_EQUAL exp','compare',3,'p_compare','Parser.py',294),
  ('compare -> exp LESSER_EQUAL exp','compare',3,'p_compare','Parser.py',295),
  ('exp -> termino','exp',1,'p_exp','Parser.py',302),
  ('exp -> termino PLUS exp','exp',3,'p_exp','Parser.py',303),
  ('exp -> termino MINUS exp','exp',3,'p_exp','Parser.py',304),
  ('termino -> factor','termino',1,'p_termino','Parser.py',311),
  ('termino -> factor TIMES termino','termino',3,'p_termino','Parser.py',312),
  ('termino -> factor DIVIDE termino','termino',3,'p_termino','Parser.py',313),
  ('termino -> factor MODULE termino','termino',3,'p_termino','Parser.py',314),
  ('factor -> variable','factor',1,'p_factor','Parser.py',321),
  ('factor -> O_PARENTHESIS expr C_PARENTHESIS','factor',3,'p_factor','Parser.py',322),
  ('factor -> llamada','factor',1,'p_factor','Parser.py',323),
  ('factor -> cte','factor',1,'p_factor','Parser.py',324),
  ('factor -> PLUS cte','factor',2,'p_factor','Parser.py',325),
  ('factor -> MINUS cte','factor',2,'p_factor','Parser.py',326),
  ('cte -> CTE_INT','cte',1,'p_cte','Parser.py',333),
  ('cte -> CTE_FLOAT','cte',1,'p_cte','Parser.py',334),
  ('epsilon -> <empty>','epsilon',0,'p_epsilon','Parser.py',341),
]
